...
class PanAndZoom(ZoomOnWheel):
    ...
    def _on_mouse_press(self, event):
        ...
        if event.button in (MouseButton.RIGHT, MouseButton.MIDDLE):  # Start
            x_axes, y_axes = self._axes_to_update(event)
            if x_axes or y_axes:
                self._axes = x_axes, y_axes
                self._pressed_button = event.button

                if self._pressed_button == MouseButton.RIGHT:  # pan
                    self._pan(event)
                elif self._pressed_button == MouseButton.MIDDLE:  # zoom area
